...
export const AgentSkillsSetupModal = ({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`${t("skills.setupBtn")} ${skill.name}`}
      onClick={onClose}
    >
      <div
        ...
        <div className="space-y-3 px-6 pb-3 text-[11px] text-muted-foreground">
          ...
          <div className="space-y-2 rounded-md border border-border/60 bg-surface-1/65 px-3 py-3">
            {installOption ? (
              ...
              >
                {busyForSkill ? t("skills.working") : installOption.label}
              </button>
            ) : null}
            <button
              type="button"
            ...
            >
              {busyForSkill
                ? t("skills.working")
                : skill.disabled
                  ? t("skills.enableGlobally")
                  : t("skills.disableGlobally")}
            </button>
            {skill.primaryEnv ? (
              <>
                <input
                  ...
                  placeholder={t("skills.setApiKey").replace("{0}", skill.primaryEnv)}
                  aria-label={`${t("skills.setApiKey").replace("{0}", "")} ${skill.name}`}
                />
                <button
                  type="button"
                  className="ui-btn-secondary w-full px-3 py-2 text-[10px] font-medium disabled:cursor-not-allowed disabled:opacity-65"
                ...
                >
                  {busyForSkill ? t("skills.working") : t("skills.saveApiKey").replace("{0}", skill.primaryEnv)}
                </button>
              </>
            ) : null}
            {canDeleteSkill ? (
              <button
                ...
                onClick={() => {
                  const approved = window.confirm(
                    t("skills.removeConfirm").replace("{0}", skill.name)
                  );
                  if (!approved) {
                    return;
                  }
